...
class BME680(BME680Data, I2C):
	...
	def __init__(self, i2c_addr=BME_680_BASEADDR, gas_measurement=True, qv_temp=None, \
		...
		self.set_temperature_oversample(OS_4X)
...
#			Log("BME680: Calculating gas baseline for 1 minute")
			while curr_time - start_time < burn_in_time:
#				print(str(curr_time - start_time) + " < " + str(burn_in_time))
				curr_time = time.time()
			...
			self.data.gas_baseline = sum(burn_in_data[-10:]) / 10.0
